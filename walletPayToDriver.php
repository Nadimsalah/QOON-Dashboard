...
// Pagination and Search for Drivers
...
$where = "1=1";
if($searchName != '') {
    $where .= " AND (FName LIKE '%$searchName%' OR LName LIKE '%$searchName%' OR DriverID = '$searchName')";
}

// Ensure the schema explicitly supports Balance or fallback to 0 in SQL if Drivers doesn't hold Balance
$resDrivers = mysqli_query($con, "
    SELECT DriverID, FName, LName, PersonalPhoto, CreatedAtDrivers 
    FROM Drivers 
    WHERE $where 
    ORDER BY CreatedAtDrivers DESC 
...
<head>
    ...
    <title>Deliveries Financial Portfolios | QOON</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Inter', sans-serif; }
        ...
        .sn-btn.active { border-color: var(--accent-blue); background: rgba(0, 122, 255, 0.05); }
        .sn-btn.active h3 { color: var(--accent-blue); }
        ...
        <main class="main-panel">
            <header class="header">
                <div class="breadcrumb">
                    ...
                    <span>/</span>
                    <span style="color: var(--accent-purple);">Financial Portfolios (Deliveries Segment)</span>
                </div>
            </header>
            ...
            <div class="sub-nav">
                <a href="walletPayToUser.php" class="sn-btn">
                ...
                <a href="walletPayToDriver.php" class="sn-btn active">
            ...
            <div class="table-container">
                <div class="table-head">
                    <h2 style="font-size:18px; font-weight:800;"><i class="fas fa-motorcycle" style="color:var(--accent-blue);"></i> Delivery Portfolios Log</h2>
                    <form class="search-box" method="GET">
                        <i class="fas fa-search" style="color:var(--text-gray);"></i>
                        <input type="text" name="name" placeholder="Search Driver ID or Name..." value="<?= htmlspecialchars($searchName) ?>">
                    </form>
                </div>
                <table>
                    <thead>
                    ...
                    <tbody>
                        <?php if (count($driversList) === 0): ?>
                        <tr><td colspan="4" style="text-align:center; padding:30px; color:var(--text-gray);">No driver balances matched the constraints.</td></tr>
                        <?php endif; ?>
                        
                        <?php foreach($driversList as $u): ?>
                        <tr>
                            <td><span style="font-weight: 800; color: var(--accent-blue); background: rgba(0, 122, 255, 0.1); padding: 5px 12px; border-radius: 8px; font-size: 12px;">#<?= $u['DriverID'] ?></span></td>
                            <td>
                                <a href="driver-profile.php?id=<?= $u['DriverID'] ?>" class="part-node">
                                    <img src="<?= htmlspecialchars($u['PersonalPhoto']) ?>" onerror="this.src='images/ensan.jpg'">
                                    <span><?= htmlspecialchars($u['FName'] . ' ' . $u['LName']) ?></span>
                                </a>
                            </td>
                            <td><span class="rev-amt" style="color:var(--text-gray);opacity:0.6;">0.00 MAD</span></td>
                            <td style="color:var(--text-gray); font-size:13px;"><?= $u['CreatedAtDrivers'] ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                ...
                <div class="pagination">
                    <span>Listing active Delivery balances</span>
                    <div class="page-ctrls">
                        ...
                        <a href="?name=<?= urlencode($searchName) ?>&Page=<?= $page + 1 ?>" class="page-btn <?= count($driversList) < $limit ? 'disabled' : '' ?>">Next <i class="fas fa-chevron-right"></i></a>
                    </div>
                </div>
            </div>
