...
$resShops = mysqli_query($con, "
    SELECT ShopID, ShopName, ShopLogo, Balance, CreatedAtShops 
    FROM Shops 
    WHERE $where 
    ORDER BY CreatedAtShops DESC 
...
<head>
    ...
    <title>Seller Financial Portfolios | QOON</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Inter', sans-serif; }
        ...
        .sn-btn.active { border-color: var(--accent-orange); background: rgba(245, 158, 11, 0.05); }
        .sn-btn.active h3 { color: var(--accent-orange); }
        ...
        <main class="main-panel">
            <header class="header">
                <div class="breadcrumb">
                    ...
                    <span>/</span>
                    <span style="color: var(--accent-purple);">Financial Portfolios (Sellers Segment)</span>
                </div>
            </header>
            ...
            <div class="sub-nav">
                ...
                <a href="walletPayToSeller.php" class="sn-btn active">
                    <h3>Sellers Money</h3>
            ...
            <div class="table-container">
                <div class="table-head">
                    <h2 style="font-size:18px; font-weight:800;"><i class="fas fa-store" style="color:var(--accent-orange);"></i> Seller Portfolios Log</h2>
                ...
                <table>
                    <thead>
                        <tr>
                            <th>Seller ID</th>
                            <th>Shop Entity</th>
                            <th>Store Account Balance</th>
                            <th>Date Joined</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if (count($shopsList) === 0): ?>
                        <tr><td colspan="4" style="text-align:center; padding:30px; color:var(--text-gray);">No seller balances matched the constraints.</td></tr>
                        <?php endif; ?>
                        
                        <?php foreach($shopsList as $u): ?>
                        <tr>
                            <td><span style="font-weight: 800; color: var(--accent-blue); background: rgba(0, 122, 255, 0.1); padding: 5px 12px; border-radius: 8px; font-size: 12px;">#<?= $u['ShopID'] ?></span></td>
                            <td>
                                <a href="shop-profile.php?id=<?= $u['ShopID'] ?>" class="part-node">
                                    <img src="<?= htmlspecialchars($u['ShopLogo']) ?>" onerror="this.src='images/placeholder.png'">
                                    <span><?= htmlspecialchars($u['ShopName']) ?></span>
                                </a>
                            </td>
                            ...
                            <td style="color:var(--text-gray); font-size:13px;"><?= $u['CreatedAtShops'] ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                ...
                <div class="pagination">
                    <span>Listing active Seller balances</span>
